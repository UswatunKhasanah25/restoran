    ...
    <section class="content">
        <div class="box box-primary">
            <div class="box-header">
                <h3 class="box-title">Complete Orders</h3> <br>
                <a href="/Laporan/print" class="btn btn-primary" target="_blank">CETAK PDF</a>
                ...
                <table id="order-table" class="table table-bordered table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><a href="{{ route('customer.show', $order->user_id) }}">{{ $order->user->name }}</a>
                            </td>
                            <td>{{ $order->makanan->nama }}</td>
                            <td>{{ $order->qty }} Porsi</td>
                            <td>Rp. {{ number_format($order->total,0,',','.') }}</td>
                            <td><img src="{{asset('storage/'.$order->transfer)}}" alt=""></td>
                            <td><label class="label label-success">Complete</label></td>
                            <td class="text-right" style="width: 180px">
                                <button class="btn btn-primary btn-sm btn-flat btn-detail" data-toggle="modal"
                                    data-target="#modal-detail" data-route="{{ route('detail-order', $order->id) }}">
                                    <i class="fa fa-eye"></i> <strong>DETAIL</strong>
                                </button>
                                <button class="btn btn-danger btn-sm btn-flat btn-delete"
                                ...
                                <form id="remove-order-{{ $order->id }}"
                                    action="{{ route('remove-order', $order->id) }}" method="POST">
                                    {{ csrf_field() }}
                                </form>
                            </td>
                        </tr>
                ...
                <div class="modal-dialog">
                    <div class="box box-primary">
                        <div class="box-header with-border">
                            <h3 class="box-title" align="center"><i class="fa fa-fw fa-info-circle"></i> Order Detail
                            </h3>
                            <div class="box-tools pull-right">
...
<script src="{{ asset('assets/dashboard/bower_components/datatables.net-bs/js/dataTables.bootstrap.min.js') }}">
</script>
<!-- PNotify -->
