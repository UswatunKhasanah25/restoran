...
Route::group(['middleware' => ['auth', 'role:customer']], function () {
    Route::get('/order/{id}', 'PemesananController@getPemesanan')->name('getPemesanan');
	Route::post('/order', 'PemesananController@postPemesanan')->name('postPemesanan');
	Route::get('/order-list', 'PemesananController@listPemesanan')->name('listPemesanan');
	Route::get('/order/{id}/detail', 'PemesananController@detailPemesanan')->name('detailPemesanan');
	Route::post('/order/{id}/cancel', 'PemesananController@cancelPemesanan')->name('cancelPemesanan');
	Route::get('/order-success', 'PemesananController@successPemesanan')->name('successPemesanan');
}); 
...
Route::group(['prefix' => 'admin'], function () {
    ...
    Route::delete('/destroy', 'MejaController@destroy')->name('meja.destroy');
    Route::delete('/destroy{id}', 'MejaController@destroy')->name('meja.destroy');
    ...
    //pemesanan
    Route::get('/order/all', 'PemesananController1@index')->name('all-order');
	Route::get('/order/pending', 'PemesananController1@pending')->name('pending-order');
	Route::get('/order/{id}/detail', 'PemesananController1@detail')->name('detail-order');
	Route::post('/order/{id}/verify', 'PemesananController1@verify')->name('verify-order');
	Route::post('/order/{id}/remove', 'PemesananController1@remove')->name('remove-order');
